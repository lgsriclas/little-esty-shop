...
RSpec.describe InvoiceItem, type: :model do
  before :each do |x|
    unless x.metadata[:skip_before]
      ...
      @ii_9 = InvoiceItem.create!(quantity: 16, unit_price: 5, status: 2, item_id: @item_2.id, invoice_id: @invoice_5.id)
      ...
      @bd_2 = BulkDiscount.create!(quantity_threshold: 12, percent_discount: 30, merchant_id: @merchant_1.id)
    end
  end
  ...
  describe 'class methods' do
    it 'returns item revenue top 5' do
      expect(InvoiceItem.item_revenue).to eq([@item_2.id, @item_1.id, @item_6.id, @item_4.id, @item_3.id, @item_5.id, @item_7.id])
    end
    ...
    it 'returns an empty array is invoices are complete', skip_before: true do
      merchant = Merchant.create!(name: "Larry's Lucky Ladles")
      item = Item.create!(name: "Star Wars Ladle", description: "May the soup be with you", unit_price: 10, merchant_id: merchant.id)
      customer = Customer.create!(first_name: "Sally", last_name: "Brown")
      invoice = Invoice.create!(status: 1, customer_id: customer.id)
      ii = InvoiceItem.create!(quantity: 5, unit_price: 10, status: 2, item_id: item.id, invoice_id: invoice.id)
      transaction = Transaction.create!(credit_card_number: "5522 3344 8811 7777", credit_card_expiration_date: "2025-05-17", result: 0, invoice_id: invoice.id)
      expect(InvoiceItem.incomplete_invoices).to eq([])
    end
  end

  describe 'bulk discounts' do
    it 'can determine if a discount should be applied to an invoice' do
      expect(@ii_8.discount).to eq(@bd_1)
      expect(@ii_9.discount).to eq(@bd_2)
      expect(@ii_2.discount).to eq(nil)
      expect(@ii_1.discount).to eq(nil)
    end

    it 'can calculate invoice item revenue including discounts' do
      expect(@ii_2.revenue).to eq(60)
